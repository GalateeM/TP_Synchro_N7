...
uint16_t dataFrameLength = DATA_FRAME_LENGTH;
uint8_t ackFrame[ACK_FRAME_LENGTH];
uint16_t ackFrameLength = ACK_FRAME_LENGTH;
uint8_t  retries = 0;
...
{  
     ...
     */
    decaduino.setRxBuffer(rxBuffer, &rxBufferLength);
...
void loop()
{
    ...
    uint8_t tab[5] = {0, 0, 2, 32, 88};
    tab[0] = panId;
    tab[1] = seqNumber;
    Serial.print("[SEND] #"); Serial.println(seqNumber);
    decaduino.plmeRxDisableRequest();
    decaduino.pdDataRequest(tab, 5);
    decaduino.plmeRxEnableRequest();
    ...
    while (!validFrameReceived && now < start + timeout && retries <= 3)
    {
        decaduino.engine();
    ...
    if (start + timeout > now) Serial.println("Timeout exceeded...");
    if (retries >= 3) {
        Serial.println("Max retries without ACK...");
        ++seqNumber;
        retries = 0;
    }
    /* -------------------------------------------------------------------------
...
void simpleDataAckProcotol(const uint64_t& timeout, const uint8_t& nbRetriesAllowed)
{
    ...
    while(true)
    {
            ...
            && nbRetriesAllowed > nbRetriesAttempted)
        {
            ...
            sendFrame(dataFrame, dataFrameLength);
            ++nbRetriesAttempted;
        }
        ...
        if (decaduino.rxFrameAvailable())
        {
            ...
            if (rxBuffer[1] == panId)
            {
                if (rxBuffer[0] == uint8_t(0)) // DATA frame
                {
                    ...
                    sendFrame(ackFrame, ackFrameLength);
